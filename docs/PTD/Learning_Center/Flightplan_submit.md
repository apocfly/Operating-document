...
为确保本次教程的内容清晰，以及系统正常、详细的支持，我们将会使用飞控v1.0版本进行演示，Swift、飞控v2.0，对于部分内容的填写较为模糊，故不进行讨论。[点此提交计划](https://v1.pilot.apocfly.com/flight-plan)

此外，由于网页布局，我们将飞行计划的填写分为上半部分、下半部分。



## 上半部分

![./assets/Snipaste_2026-02-16_14-05-51.png](./assets/Snipaste_2026-02-16_14-05-51.png)
...
- 尾流分类：此处按照飞机最大起飞重量进行填写，[在此处查询](https://www.icao.int/publications/doc-8643-aircraft-type-designators/search)

	!!! Note "注意了解"
	
		填写的为：WTC（ICAO**四类**），而不是RECAT-CN分类。可[在此](./Aircraft_type)，了解更多
...
- 巡航真空速（knots）：按照飞机正常的巡航真空速填写（不是马赫数），正常情况下，应填写实际的。但，如果您不知道，也可填写最大巡航速度，[在此查询](https://skybrary.aero/aircraft-types)，管制员对此部分内容不进行检查
...
- 航路：航路可查询[本文](./Airlines_flight_route.md)
...
![](./assets/Snipaste_2026-02-16_14-08-16.png)

以下内容除通讯以外，均为非必要填写，仅为提升模拟沉浸感使用：

- `PBN/`：PBN码，通用代码（A1B1C1D1L1O2S2）。同上上，将在后文阐述
- `NAV/`：包括PBN多余的还有自己独属的。同上上，将在后文阐述
- `DAT/`：其他数据应用
- `SUR/`：其他监视能力
- `DOF/`：飞行计划执行日期，UTC时间
- `REG/`：飞机注册号，如：B-1234
...
- `COM/`：其他通信能力
- `EET/`：预计经过时间
- `RMK/`：Remarks（备注）填写额外信息，可以备注TCAS*
	- 例如：EXERCISE FLIGHT 表示本次航班为训练飞行等
- 交流类型：
	- 双向语音（V）
	- 仅接受文字（R）
	- 仅文字（T）
